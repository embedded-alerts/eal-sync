...
Secrets for `embedded-alerts/eal-sync` are **committed, encrypted**, with [sops] + [age],
...
- **No multi-line values.** A PEM must be a single line with `\n` escapes:
  `JWT_PRIVATE_KEY="<PEM header>\n<base64 body>\n<PEM footer>\n"`
  The angle-bracket values are explanatory placeholders, not a usable key.
  Do not paste private-key material into documentation; edit it only through
  the encrypted-secret workflow described above.
...
```sh
just env-docker-run dev ghcr.io/embedded-alerts/eal-sync:dev
